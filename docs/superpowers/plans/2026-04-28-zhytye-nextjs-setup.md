# Житездатність — Next.js Setup Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Convert the single JSX simulator file into a working Next.js 14 project with Tailwind CSS, lucide-react, and a secure server-side Anthropic API proxy.

**Architecture:** Next.js 14 App Router. The simulator component runs as a Client Component. The Anthropic API key lives only in the server-side route handler — never sent to the browser. The frontend fetches `/api/generate-events` (internal) instead of calling Anthropic directly.

**Tech Stack:** Next.js 14, React 18, Tailwind CSS 3, lucide-react, Node.js fetch (in route handler)
...
```bash
cd /Users/romkravets/Documents/GitHub/zhytye
npm install
```
...
git commit -m "feat: add package.json with Next.js dependencies"
```
...
```bash
git add next.config.js app/layout.jsx
git commit -m "feat: add Next.js config and root layout"
```
...
```bash
git add app/api/generate-events/route.js
git commit -m "feat: add Anthropic API proxy route"
```
...
```bash
git add components/ZhyteSimulator.jsx
git commit -m "feat: add ZhyteSimulator component with secure API proxy integration"
```
...
git commit -m "feat: add home page rendering ZhyteSimulator"
```
...
```bash
git add .gitignore
git commit -m "chore: add .gitignore (excludes node_modules, .next, .env.local)"
```

Note: `.env.local` is intentionally NOT committed — it's in `.gitignore`.
...
Expected: Setup screen with "Житездатність" heading, all toggle sections visible, budget preview at bottom.

- [ ] **Step 3: Test fallback (no API key)**

If `.env.local` doesn't have a valid key yet, click "почати симуляцію" — the app should still work using `FALLBACK_EVENTS`. The loading screen should appear briefly then the simulation starts.

- [ ] **Step 4: Test with real API key (optional)**

Add a valid `ANTHROPIC_API_KEY` to `.env.local`, restart the dev server (`Ctrl+C` then `npm run dev`), and run the simulation again. Check browser DevTools → Network tab: the request goes to `/api/generate-events` (localhost), not `api.anthropic.com`. The API key should not appear in any browser request.

- [ ] **Step 5: Final commit**

```bash
git add .
git commit -m "feat: complete Next.js project setup for Житездатність simulator"
```
